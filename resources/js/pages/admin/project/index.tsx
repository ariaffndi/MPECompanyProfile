...
export default function Project() {
   ...
   const { search, setSearch, sortOrder, toggleSort, filtered } = useSearchSort(project.data, (projectItem) => projectItem.project_name);
   ...
   const handleExportCSV = () => {
      const headers = ['project_name', 'client_id', 'category_id', 'location', 'year', 'value', 'description'];
      const rows = project.data.map((projectItem, i) => [i + 1, projectItem.project_name, projectItem.description.replace(/\n/g, ' ')]);
            ...
            <div className="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
               ...
               <div className="rounded-box border-base-content/5 w-full overflow-x-auto border">
                  <table className="table-pin-rows table min-w-full text-center">
                        ...
                        <tbody>
                           {filtered.map((projectItem, index) => (
                              <tr
                                    key={projectItem.id}
